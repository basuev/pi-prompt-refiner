...
function refinementInput(prompt: string, conversationSummary?: string): string {
	if (!conversationSummary) return prompt;
...
async function refinePrompt(
	...
	const contextualInput = refinementInput(input, conversationSummary);
...
export default function promptRefiner(pi: ExtensionAPI) {
	pi.registerTool({
		name: "refine_prompt",
		label: "Refine Prompt",
		description: "Refine prompt text with GPT-5.6 Luna through the OpenAI Codex subscription, using high reasoning with Luna-only fallback.",
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			onUpdate?.({
				content: [{ type: "text", text: "Preparing context for GPT-5.6 Luna..." }],
				details: { model: `${PROVIDER}/${MODEL_ID}`, efforts: LUNA_EFFORTS.map(({ level }) => level) },
			});
			...
			const result = await refinePrompt(pi, params.prompt, conversationSummary, signal);
			return {
	...
	pi.registerCommand("refine-prompt", {
		...
		handler: async (args, ctx) => {
			...
			let input = args.trim();
			if (!input) {
				const entered = await ctx.ui.editor("Prompt to refine", "");
			...
			const result = await ctx.ui.custom<RefinementResult | null>((tui, theme, _keybindings, done) => {
				const loader = new BorderedLoader(tui, theme, "Preparing prompt with GPT-5.6 Luna...");
				loader.onAbort = () => done(null);
				summarizeSession(pi, ctx, input, false, loader.signal)
					.then((conversationSummary) => refinePrompt(pi, input, conversationSummary, loader.signal))
					.then(done)
					.catch((error) => {
			...
			ctx.ui.notify(`Prompt ready: GPT-5.6 Luna, ${result.effort.level}.`, "info");
		},
	});
